...
import { ParseError, parseStylesFile } from "../lib/CetzitParser";
...
const StyleEditor = ({ initialContent }: StyleEditorProps) => {
  ...
  const parsed = parseStylesFile(initialContent.document);
  ...
  const tryParseStyles = (tikz: string) => {
    const parsed = parseStylesFile(tikz);
  ...
  const newStyle = (isEdgeStyle: boolean) => {
    ...
    if (isEdgeStyle) {
      d = d.setKind("edge").setProperty("stroke", "black + 0.4pt");
    } else {
      d = d.setKind("node").setProperty("shape", `"pill"`);
    }
