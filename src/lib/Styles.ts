...
class Styles {
  ...
  public cetzitWithPosition(
    ...
    this.styles.forEach(style => {
      if (style.name !== "none") {
        result += `#let ${style.name} = (`;
        if (style.name === styleName) {
          const lines = result.split("\n");
          position = { line: lines.length - 1, column: lines[lines.length - 1].length };
        }
        const entries: string[] = [];
        for (const key of style.propertyKeys) {
          const v = style.property(key);
          entries.push(v === undefined || v === "" ? key : `${key}: ${v}`);
        }
        result += entries.join(", ");
        result += ")\n";
      }
  ...
  public tikzWithPosition(
    ...
    return this.cetzitWithPosition(styleName);
  }
